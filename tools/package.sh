...
for fp in "$TEMPLATE_DIR"/*.yaml; do
  ...
  if [ $file != 'main.yaml' ]; then
    aws cloudformation package --s3-bucket $BUCKET --force-upload --template-file "$fp" --output-template-file "$TEMPLATEPACKAGED_DIR/$file" --region $REGION
  fi
done

## main template packaging
aws cloudformation package --s3-bucket $BUCKET --force-upload --template-file "$TEMPLATE_DIR/main.yaml" --output-template-file "$TEMPLATEPACKAGED_DIR/main.yaml" --region $REGION

cat "$TEMPLATE_DIR/main.yaml"
